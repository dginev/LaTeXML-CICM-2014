...
% Silviu: The paper constantly mentions that one of the purposes of LaTeXML is to preserve macro semantics. Isn't it beyond that? Something like ``infer macro semantics'' might be closer to the real purpose (although infer is a big word). Think about generating content mathml: in raw tex, the formula doesn't have any semantic markup - it only makes sense in the mind of the one who wrote the document.
...
% Silviu: Perhaps we should make it more clear that, concerning graphics, the purpose is to write a universal SVG generator, for basic things like line, point, etc. (which will always make sense, irrespective of the package) and further package-specific conversions to that basic layer? (is that still the purpose?).
\section{Outlook}
